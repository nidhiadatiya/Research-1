				...
				<table class="table table-borderless" id="projects" border="1">
					<thead>
							<tr>
									...
									<th class="tbheader" colspan="5"><h4>Participants</h4></th>
					...
					<tbody>
						...
						function getPartiDesc($id, $tabledatadetails){
							$str = "";
						...
						function getPartiData($id, $tabledatadetails, $type){
							$max = 30;
							$str = "";
							foreach($tabledatadetails as $value){
								if($value->id == $id){
									if($type == "pid"){
										...
										if($value->$type == 'Project_Coordinator')	$value->$type = 'Mentor';
										if(strlen($value->$type) > $max){
											$str .= substr($value->$type, 0, $max) . "..." . "<br>";
										}else{
											$str .= $value->$type . "<br>";
										}
									}
						...
						foreach($tabledata as $id => $value){
							if($value->year == $year){
								...
								echo "<td class='tbpartiid'>".getPartiData((int) $value->id, $tabledatadetails, "pid")."</td>";
								echo "<td class='tbparti' style='width: 250px;overflow:hidden'>".getPartiData((int) $value->id, $tabledatadetails, "name")."</td>";
								echo "<td class='tbparti'>".getPartiData((int) $value->id, $tabledatadetails, "role")."</td>";
								echo "<td class='tbparti'>".getPartiDesc((int) $value->id, $tabledatadetails)."</td>";
								echo "<td class='tbpartidept'>".getPartiData((int) $value->id, $tabledatadetails, "branch")."</td>";
